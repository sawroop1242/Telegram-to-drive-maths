#!/usr/bin/env python3
"""Download authorized Maths VOD VIDEO/PDF entries and upload them through rclone."""
from __future__ import annotations
import argparse, hashlib, json, os, re, shutil, subprocess, sys, time
from pathlib import Path
import requests
from vod_manifest import load_rows as load_manifest_rows
...
def load_rows(csv_path: Path | None = None) -> list[dict]:
    """Load the embedded 765-row manifest; optionally fall back to a CSV."""
    try:
        rows = load_manifest_rows()
        if len(rows) != 765:
            raise ValueError(f"Embedded manifest contains {len(rows)} rows; expected 765")
        return rows
    except Exception as manifest_error:
        if csv_path is None or not csv_path.exists():
            raise RuntimeError(f"Embedded VOD manifest could not be loaded: {manifest_error}")
        import csv
...
def process_row(row: dict, state: dict, dry_run=False) -> bool:
    ...
    if item_type not in {"VIDEO", "PDF"}:
        return True
    ext, subfolder = ((".mp4", "Videos") if item_type == "VIDEO" else (".pdf", "PDFs"))
    key = f"{row.get('#', 'unknown')}|{url}"
    if state["completed"].get(key):
        print(f"[DONE] row {row.get('#')}: {title}")
        return True
    ...
    try:
        ...
        save_state(state)
        print(f"[OK] uploaded: {drive_path}")
        return True
    except Exception as e:
        state["failed"][key] = {"title": row["Title"], "type": item_type, "category": row["Category"], "url": url, "error": str(e)}
        save_state(state)
        print(f"[FAIL] row {row.get('#')}: {e}", file=sys.stderr)
        return False


def main() -> int:
    p = argparse.ArgumentParser()
    p.add_argument("--csv", default=DEFAULT_CSV, help="Optional fallback CSV; embedded manifest is used first")
    p.add_argument("--only", choices=["VIDEO", "PDF", "ALL"], default="ALL")
    p.add_argument("--category")
    p.add_argument("--limit", type=int, default=0)
    p.add_argument("--dry-run", action="store_true")
    args = p.parse_args()
    if shutil.which("ffmpeg") is None or shutil.which("rclone") is None:
        print("ERROR: ffmpeg and rclone are required", file=sys.stderr)
        return 2
    rows = load_rows(Path(args.csv)); state = load_state(); selected = []
    for row in rows:
        typ = row["Type"].strip().upper()
        if args.only != "ALL" and typ != args.only:
            continue
        if args.category is not None and row["Category"] != args.category:
            continue
        selected.append(row)
    if args.limit > 0:
    ...
    ok = failed = 0
    for row in selected:
        if process_row(row, state, args.dry_run):
            ok += 1
        else:
            failed += 1
